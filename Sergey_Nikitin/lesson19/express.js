...
io.on('connection', (socket) => {
    ...
    socket.on('create', async (data) => {
        ...
        const toDoItem = new toDoListModel(data)
        const savedToDoItem = await toDoItem.save()


        socket.broadcast.emit(`created:${savedToDoItem.user}`, savedToDoItem) //Сработает у всех, кроме отправителя
        socket.emit(`created:${savedToDoItem.user}`, savedToDoItem) //Сработает только у отправителя
    })

    socket.on('delete', async (toDoItemId) => {
        console.log('Пришло событие из браузера - delete')

        const toDoItem = await toDoListModel.findOne({ _id: toDoItemId }).lean()
        await toDoListModel.findByIdAndRemove(toDoItemId, (err) => {
            if (err) {
                return
            }
            socket.broadcast.emit(`deleted:${toDoItem.user}`, toDoItemId)
            socket.emit(`deleted:${toDoItem.user}`, toDoItemId)
        })
    })
    socket.on('switch', async (toDoItemId) => {
        console.log('Пришло событие из браузера - switch')
        const toDoItem = await toDoListModel.findOne({ _id: toDoItemId }).lean()
        toDoListModel.updateOne({ _id: toDoItemId }, { checked: !toDoItem.checked }, async function (err) {
            if (!err) {
                console.log('Success!')
                // const toDolistItemNew = await toDoListModel.findById({ _id: id })
                //return res.status(200).json({ message: "Удалось обновить дело" }) 
                const toDoItemUpdt = await toDoListModel.findOne({ _id: toDoItemId }).lean()
                socket.broadcast.emit(`switched:${toDoItem.user}`, toDoItemUpdt)
                socket.emit(`switched:${toDoItem.user}`, toDoItemUpdt)
            }
            else {
