//код взят из 9 задания
//1 выполнено через fetch
//2 было и раньше
//3* сделано как пример без отрисовки
'use strict';
...
function sendRequest(url) {//3* собвенно взято из практикума
    return new Promise((resolve, reject) => {
        const xhr = new XMLHttpRequest();
        // xhr.open('GET', 'http://localhost:3000/users', false); //Синхронный запрос
...
function getProducts() {//получение и отрисовка каталога
    fetch('/product')
...
window.onload = function () {
    getProducts();//получение и отрисовка каталога
    backetCurrent.ShowBasket(basketDom);
    sendRequest('/product').then((products) => { console.log(products) });//3* как пример, отрисовывать нестал ибо через fetch проще
}
