...
import { Tooltip, Avatar, Icon, CircularProgress } from '@material-ui/core'

import './Message.scss'

export const Message = (props) => {
  const { text, bot, user, isLoading, onDelete } = props

  const [messageState, setMessageState] = useState({ hovered: false })

  const onHover = () => {
    if (bot) {
      return
    }
    setMessageState({ ...messageState, hovered: !messageState.hovered })
  }
    ...
    <div className={classes}>
      {!isLoading ? (
        <Tooltip title={!bot ? user.name : 'Бот'}>
          {!bot && messageState.hovered ? (<Avatar
            style={avatarStyle}
