...
const modal = {
    ...
    addImg(src, alt) {
        ...
        imgItem.src = src;
        imgItem.alt = alt;
        imgItem.dataset.otnSrc = src;
        this.divImgContainer.appendChild(imgItem);
        this.imgList.push(src);
        imgItem.addEventListener('click', function (event) { //По клику на миниатюре выводим большое изображения
            modal.curImg(event.target.dataset.otnSrc, 'Изображение товара');
        });
    },
    curImg(src, alt) {
        ...
        this.currenImgSrc = src;
    },
    listingImg(direction) {
        //Надо получить список адресов картинок и листать их        
        let curPos = this.imgList.indexOf(this.currenImgSrc);
        let newPos = curPos;
        if (newPos < 0) {
            newPos = 0;
        }
            ...
            switch (direction) {
                case 'Left':
                    if (newPos === 0)
                        newPos = this.imgList.length - 1;
                    else newPos--;
                    break;
                case 'Right':
                    if (newPos === this.imgList.length - 1)
                        newPos = 0;
                    else newPos++;
                    break;
            }
        }
        this.curImg(this.imgList[newPos], 'Изображение товара');
    },
    closeModal() {
    ...
    pressKey(event) {
        switch (event.key) {
            case 'ArrowLeft':
                this.listingImg('Left');
                break;
            case 'ArrowRight':
                this.listingImg('Right');
                break;
        }
    }
